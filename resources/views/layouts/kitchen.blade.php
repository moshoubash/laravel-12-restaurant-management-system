...
<body class="font-sans antialiased bg-surface text-on-surface">
    ...
    <div class="flex h-screen overflow-hidden">
        @include('components.sidebar.kitchen')
                        ...
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-surface-100 transition-colors">
                                <div class="avatar avatar-sm">
                                    <div class="avatar-fallback">{{ substr(auth()->user()?->name ?? 'K', 0, 2) }}</div>
                                </div>
                                <span class="hidden sm:inline text-sm font-medium text-surface-700">{{ auth()->user()?->name ?? 'Kitchen' }}</span>
