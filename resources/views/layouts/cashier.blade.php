...
<head>
    ...
    <title>{{ config('app.name', 'RESaaS') }} — Cashier</title>
                        ...
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-surface-100 transition-colors">
                                <div class="avatar avatar-sm">
                                    <div class="avatar-fallback">{{ substr(auth()->user()?->name ?? 'C', 0, 2) }}</div>
                                </div>
                                <span class="hidden sm:inline text-sm font-medium text-surface-700">{{ auth()->user()?->name ?? 'Cashier' }}</span>
